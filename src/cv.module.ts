...
@Module({
  controllers: [EducationsController, ExperiencesController, DocumentationsController, CvController],
  imports: [
    AuthenticationModule,
    CvConfigurationModule,
    PaginationModule,
    TypeOrmModule.forFeature([Education, Experience, Documentation, Cv]),
    TypeOrmModule.forRootAsync({
      ...
      useFactory(postgresUri: string) {
        return {
          url: postgresUri,
          type: 'postgres',
          entities: [Education, Experience, Documentation, Cv],
          synchronize: true
        };
      },
      inject: [POSTGRES_URI]
  ...
  providers: [CvOpenapi, EducationsService, PermissionsService, ExperiencesService, DocumentationsService, CvService]
})
